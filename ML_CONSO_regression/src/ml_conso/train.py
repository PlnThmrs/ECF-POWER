...
def train_base_models(X_train, y_train, X_test, y_test):
    ...
    for name, model in models.items():
        ...
        metrics = evaluate_model(model, X_test, y_test) #Évaluation du modèle sur les données de test
        # renvoie un dictionnaire de métriques d'évaluation
        results[name] = { #Stockage du modèle et de ses métriques d'évaluation dans le dictionnaire des résultats
            "model": model,
